...
use crate::objects::{remove_player, ObjectModel, Position};
    ...
    if let Err(e) = handle_connection(peer, stream).await {
        match e {
            Error::ConnectionClosed | Error::Protocol(_) | Error::Utf8(_) => {
                Log::new(
                    LogLevel::Info,
                    peer.clone().ip().to_string() + " disconnected",
                );
...
async fn handle_connection(peer: SocketAddr, stream: TcpStream) -> Result<(), Error> {
    ...
    Log::new(
        LogLevel::Info,
        "New WebSocket connection: ".to_string() + &*peer.ip().to_string(),
    );
...
async fn authenticate_peer(peer: SocketAddr) -> Result<(), Error> {
    ...
    game_state
        .get_mut(&*id)
        .unwrap()
        .set_pos(Position::new(rng, 1, rng))
        .await;
    drop(game_state);
...
async fn main() {
    ...
    while let Ok((stream, _)) = listener.accept().await {
        ...
        Log::new(
            LogLevel::Info,
            "Peer address: ".to_string() + &*peer.ip().to_string(),
        );
